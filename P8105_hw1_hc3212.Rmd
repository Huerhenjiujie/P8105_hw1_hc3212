...
The **Penguins** dataset contains the following variables: `r ls(penguins)`.
	...
	Bill Depth (mm) is a `r class(pull(penguins, bill_depth_mm))` variable. The mean bill depth (mm) is `r mean(pull(penguins, bill_depth_mm), na.rm=TRUE)`. Values for bill depth range from `r range(pull(penguins, bill_depth_mm), na.rm=TRUE)` (min, max). ```{r descriptdata}
char_class = ls(penguins)
while (char_class != null){
char_a = char_class[1],
if 
